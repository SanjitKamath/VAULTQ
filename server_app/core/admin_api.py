...
def provision_doctor(name: str):
    # Generate random ID and Temp Password
    doc_id = "doc_" + secrets.token_hex(4)
    ...
    return {"doctor_id": doc_id, "temp_password": temp_pass}

@router.delete("/doctors/{doctor_id}")
def delete_doctor(doctor_id: str):
    if doctor_id in db.doctors:
        del db.doctors[doctor_id]
        db.save_db() # Permanent removal
        return {"message": "Doctor wiped."}
